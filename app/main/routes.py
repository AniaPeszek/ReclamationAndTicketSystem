...
def index():
    if current_user.is_authenticated:
        open_tickets = (
            ...
            .filter_by(assigned_employee_id=current_user.id)
            .filter_by(status=0)
...
def get_tickets_chart_data():
    ...
    for i in range(number_of_months):
        ...
        tickets_in_month = (
            db.session.query(Ticket)
            .filter(Ticket.ticket_assigned == current_user)
            .filter(Ticket.finished_date <= start)
            .filter(Ticket.finished_date >= end)
            .count()
        )
...
def get_reclamations_chart_data(model="All Models"):
    ...
    if model == "All Models":
        for i in range(number_of_months):
            labels.append(start.strftime("%B"))
            recl_in_month = (
                db.session.query(Reclamation)
                .filter(Reclamation.informed_date <= start)
                .filter(Reclamation.informed_date >= end)
                .count()
            )
        ...
        reclamation_part_join = (
            db.session.query(PartDetails)
            .filter(PartDetails.part_no_id == part_no.id)
            .join(Reclamation)
        )
        for i in range(number_of_months):
            labels.append(start.strftime("%B"))
            recl_in_month = (
                reclamation_part_join.filter(Reclamation.informed_date <= start)
                .filter(Reclamation.informed_date >= end)
                .count()
            )
            data.append(recl_in_month)
...
def get_pie_chart_data():
    ...
    models = (
        db.session.query(PartNo.model, func.count(PartNo.model))
        .join(PartDetails)
        .join(last_month_reclamations, PartDetails.reclamation_p_sn)
        .group_by(PartNo.model)
        .all()
    )
