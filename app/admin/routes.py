...
class UserAdminView(sqla.ModelView):
    ...
    form_columns = ['first_name', 'last_name', 'username', 'email', 'team', 'supervisor', 'position', 'subordinate',
                        'role', 'reclamation_req', 'ticket_ass', 'part_no_person']
    column_auto_select_related = True
    column_searchable_list = ('username', 'last_name', 'first_name', 'email', 'position')
...
class ReclamationAdminView(sqla.ModelView):
    ...
    form_args = dict(
        status=dict(
            choices=[(0, 'open'), (1, 'closed')], coerce=int
        ))
    column_auto_select_related = True
    column_filters = ['status', 'reclamation_customer.name', 'reclamation_part_sn_id.part_sn']
...
class TicketAdminView(sqla.ModelView):
    ...
    column_filters = ['creation_date', 'due_date', 'finished_date', 'status', 'reclamation.reclamation_customer',
                      'ticket_requester.last_name', 'ticket_requester.first_name',
                      'ticket_assigned.last_name', 'ticket_assigned.first_name']
    column_labels = {'reclamation.reclamation_customer': 'Customer',
...
class PartDetailsAdminView(sqla.ModelView):
    ...
    page_size = 50
    column_filters = ['part_sn', 'production_date', 'part_no.model', 'part_no.manufacturer']
    column_labels = {'part_sn': 'Part Serial Number',
                     'part_no.model': 'Model',
...
class TeamAdminView(sqla.ModelView):
    ...
    column_labels = {'team_name': 'Team name',
