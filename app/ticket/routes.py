...
from app.ticket.forms import (
    TicketForm,
    AssignedUserTicketForm,
    ReadOnlyTicketForm,
    RequesterTicketForm,
    TicketFromReclamationForm,
    CloseTicketForm,
    ReopenTicketForm,
)
...
def ticket(ticket_number):
    ...
    users_who_can_edit = [
        ...
        ticket.ticket_requester,
        ticket.ticket_assigned.team.team_leader,
    ]
    if ticket.status == 1:
        form = ReadOnlyTicketForm(
        ...
        if (
            current_user == ticket.ticket_requester
            or current_user == ticket.ticket_assigned.team.team_leader
        ):
            form = RequesterTicketForm(
                formdata=request.form,
                obj=ticket,
                assigned_employee=ticket.ticket_assigned,
                reclamation=ticket.reclamation,
