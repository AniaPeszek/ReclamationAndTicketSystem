...
def create_part_no():
    ...
    models = [
        ...
        "Chain KX X9",
        "Chain Shim X9",
        "Rear derailleur XTR-11",
        "Rear derailleur RD-M970",
    ]
    persons_in_charge = []
    for i in range(10):
    ...
    for i in range(len(models)):
        part = PartNo(
            ...
            person_in_charge=persons_in_charge[i].id,
        )
        db.session.add(part)
        db.session.commit()
...
def create_part_details(model):
    ...
    part = PartDetails(
        part_sn=part_sn,
        part_no=part_no,
        production_date=get_random_date(
            min_year=2018, min_month=1, max_year=2019, max_month=6
        ),
    )
    db.session.add(part)
    db.session.commit()
...
def create_reclamation(is_open, model, description_reclamation, tickets_descriptions):
    service_assistants = User.query.filter_by(
    ...
    for i in range(len(service_assistants)):
        ...
        recl = Reclamation(
            reclamation_requester=service_assistants[i],
        ...
        for ticket in tickets_descriptions:
            if ticket == "contact with customer":
                create_ticket(
                    ticket, recl, delay=18, customer_service=True, is_open=is_open
                )
            else:
                delay += 3
    ...
    reclamation_data = [
        {
            ...
            "tickets_descriptions": [
                "remove the tyre and straight the wheel",
                "put on the tire",
...
def create_ticket(
    description, reclamation, delay, customer_service=False, is_open=False
):
    if customer_service:
    ...
    db.session.commit()
    create_notification(
        ticket.id, assigned, requester, ticket.creation_date, reclamation
    )
